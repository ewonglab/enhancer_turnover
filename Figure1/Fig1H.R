...
enh_rt <- read.table(file = "mouse_enh_meanRT_by_germ_line_cons2sp.txt", header =T
...
dim(active_rt)
# [1] 68684     8
dim(unique(active_rt[, c("group", "id")]))
# [1] 68684     2

dim(poised_rt)
# [1] 98582     8
dim(unique(poised_rt[, c("group", "id")]))
# [1] 98582     2
...
## saving heatmaps with multiple clustering methods
#all samples rt range is wider than germline rt range
...
pdf("mouse_poised_rt_by_tissue_heatmap_matchedColour_cons2sp.pdf")
pheatmap(poised_rt.heat, cluster_rows=TRUE, cluster_cols=TRUE
         , color = pal(100), annotation_row = annotation_poised
         , breaks = rt_bins)
dev.off()

pdf("mouse_active_rt_by_tissue_heatmap_matchedColour_cons2sp.pdf")
pheatmap(active_rt.heat, cluster_rows=TRUE, cluster_cols=TRUE
         , color = pal(100), annotation_row = annotation_active
         , breaks = rt_bins)
dev.off()
