...
# RT predicted from OVARY ATAC-seq
rt <- fread('/srv/scratch/z5291980/replication_timing/data/human/replicon_ovary_hg19.bed'
                 ,header=F)#2161380       4
rt_gr <- with(rt, GRanges( V1 , IRanges( V2+1, V3 )))
names(rt) <- c('chr','start','end','rt')

dfplot <- data.frame( ID=character(), rt=numeric() )
filenames <- list.files("/srv/scratch/wonglab/data/K562_TF/hg19/" , pattern="*\\.bed", full.names=TRUE)#71 in total
...
write.table(x = mt, file = "ESC_RT_with_K562_TF.mixture_RepliconOvaryRT.txt", sep = '\t', quote = F)

comp <- read.delim('/srv/scratch/z5291980/jaspar_data/JASPAR_core_nucl_comp_and_ic.txt')#not the whole jaspar tab
...
m$mu1 <- as.numeric(m$mu1)
m$mu2 <- as.numeric(m$mu2)

cor(m$mu1, m$AT.proportion)#-0.1599932
cor(m$mu2, m$AT.proportion)#-0.05038139
cor(m$mu1, m$GC.proportion)#0.2331846
cor(m$mu2, m$GC.proportion)#0.1291272

sub <- subset(m, mu1 < mu2)

round(with(sub, cor(mu2, AT.proportion)), 2)# [1] -0.05
round(with(sub, cor(mu2, GC.proportion)), 2)# [1] 0.13
round(with(sub, cor(mu1, AT.proportion)), 2)# [1] -0.16
round(with(sub, cor(mu1, GC.proportion)), 2)# [1] 0.23

pdf('ESC_RT_K562_earlylate.AT.proportion.ovaryRT.pdf')  
ggplot(sub, aes(mu1, AT.proportion)) +
  geom_point() +  stat_smooth(method = "lm") + theme_classic()
dev.off()

pdf('ESC_RT_K562_earlylate.GC.proportion.ovaryRT.pdf')  
