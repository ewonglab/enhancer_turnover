### scatterplots replication time (germline or somatic) versus enhancer turnover
...
library(corrplot)
library("pheatmap")
library(mclust)# version 5.4.8

setwd("/srv/scratch/z5291980/replication_timing/data/mouse/hiratani")
...
#reading enhancers
mouse.enh <- read.table(file = "mouse_all_enh_byMark_type_and_tissue.bed"
...
library(ggplot2)
pdf("mouse_germlineRT_lineplot_mm10.pdf")
ggplot(data=mean_rt, aes_string(x="mean_germlineRT"
                                , y="kmeans.cluster", group=1)) +
  geom_line() + theme_classic()
dev.off()

pdf("mouse_somaticRT_lineplot_mm10.pdf")
ggplot(data=mean_rt, aes_string(x="mean_somaticRT"
                                , y="kmeans.cluster", group=1)) +
  geom_line() + theme_classic()
dev.off()

pdf("mouse_consvrecent_enh_lineplot_mm10.pdf")
ggplot(data=mean_rt, aes_string(x="cons_vs_recent_LogFC"
                                , y="kmeans.cluster", group=1)) +
  geom_line() + theme_classic()
dev.off()
