...
# Count the number of histone mark peaks overlapping genomic windows
...
# calculate the proportion of overlap with histone mark per genomic window
...
H3K27ac <- fread("ENCFF044JNJ_H3K27ac_hg19.bed.gz")
H3K4me2 <- fread("ENCFF099LMD_H3K4me2_hg19.bed.gz")
H4K20me1 <- fread("ENCFF139CKE_H4K20me1_hg19.bed.gz")
H3K9ac <- fread("ENCFF173ULG_H3K9ac_hg19.bed.gz")
H3K4me1 <- fread("ENCFF183UQD_H3K4me1_hg19.bed.gz")
H3K27me3 <- fread("ENCFF322IFF_H3K27me3_hg19.bed.gz")
H3K36me3 <- fread("ENCFF498CMP_H3K36me3_hg19.bed.gz")
H2AFZ <- fread("ENCFF624XRN_H2AFZ_hg19.bed.gz")
H3K4me3 <- fread("ENCFF737AMS_H3K4me3_hg19.bed.gz")
H3K9me1 <- fread("ENCFF652WML_H3K9me1_hg19.bed.gz")
H3K9me3 <- fread("ENCFF894VEM_H3K9me3_hg19.bed.gz")
H3K79me2 <- fread("ENCFF769WZF_H3K79me2_hg19.bed.gz")
...
for(i in 1:length(bins_with_histones_N)){
  write.table(x = bins_with_histones_N[[i]], file = paste(out_name, paste0(names(bins_with_histones_N[i]), "_N.txt"), sep = "_"), quote = F, row.names=F, sep = '\t')}

for(i in 1:length(bins_with_histones)){
  write.table(x = bins_with_histones[[i]], file = paste(out_name, paste0(names(bins_with_histones[i]), ".txt"), sep = "_"), quote = F, row.names=F, sep = '\t')}
