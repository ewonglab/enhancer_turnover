...
# Hepg2
liver_rt <- fread("GSM923446_hg19_wgEncodeUwRepliSeqHepg2WaveSignalRep1.bedGraph")
# H9 ESC
h9_rt <- fread("/g/data/zk16/cc3704/replication_timing/human/RT_H9_ESC_Ext29405702_hg19.bedgraph")
# predicted from ovary ATAC-seq
...
# predicted from testis ATAC-seq
...
# add coordinates to predicted DNA replication time
...
bins_with_liver_rt <- continuous_var_per_bin(genome_bins, liver_rt)
...
colnames(bins_with_liver_rt) <- c("bin_id", "liver_rt")
...
write.table(x = bins_with_liver_rt, file = paste(out_name, "liver_rt.txt", sep = "_"), quote = F
, row.names=F, sep = '\t')
write.table(x = bins_with_ovary_rt, file = paste(out_name, "ovary_rt.txt", sep = "_"), quote = F
, row.names=F, sep = '\t')
write.table(x = bins_with_testis_rt, file = paste(out_name, "testis_rt.txt", sep = "_"), quote = F, row.names=F
, sep = '\t')
