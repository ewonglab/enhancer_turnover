...
# proportion of overlap with ATAC-seq peaks
...
#ATAC  
atac <- fread("ENCFF631JIS_ATAC_hg19.bed")
...
write.table(x = bins_with_atac_N, file = paste(out_name, "N_ATAC.txt", sep = "_"), quote = F
, row.names=F, sep = '\t')
write.table(x = bins_with_atac, file = paste(out_name, "prop_ATAC.txt", sep = "_"), quote = F
, row.names=F, sep = '\t')
