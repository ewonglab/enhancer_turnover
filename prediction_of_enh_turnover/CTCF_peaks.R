...
# Function to count the number of CTCF peaks per genomic window
...
# Function to calculate the overlap with CTCF peaks per genomic window
...
# Read CTCF peaks
ctcf <- fread("ENCFF002CEL_CTCF_hg19.bed.gz")
...
write.table(x = bins_with_ctct_N, file = paste(out_name, "N_CTCF.txt", sep = "_"), quote = F, row.names=F, sep = '\t')
write.table(x = bins_with_ctct, file = paste(out_name, "prop_CTCF.txt", sep = "_"), quote = F, row.names=F, sep = '\t')
